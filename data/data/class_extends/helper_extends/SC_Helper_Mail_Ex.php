    ...
    function sfSendOrderMail($order_id, $template_id, $subject = '', $header = '', $footer = '', $send = true) {
        ...
        foreach ($arrOrderDetail as $key => $detail) {
            if (!empty($detail['body_color_classcategory_id'])) {
                $arrOrderDetail[$key]['body_color_classcategory_name'] = SC_Helper_DB_Ex::sfGetClassCatName($detail['body_color_classcategory_id']);
            }
            if (!empty($detail['back_color_classcategory_id'])) {
                $arrOrderDetail[$key]['back_color_classcategory_name'] = SC_Helper_DB_Ex::sfGetClassCatName($detail['back_color_classcategory_id']);
            }
            if (!empty($detail['edge_color_classcategory_id'])) {
                $arrOrderDetail[$key]['edge_color_classcategory_name'] = SC_Helper_DB_Ex::sfGetClassCatName($detail['edge_color_classcategory_id']);
            }
            if (!empty($detail['edge_size_classcategory_id'])) {
                $arrOrderDetail[$key]['edge_size_classcategory_name'] = SC_Helper_DB_Ex::sfGetClassCatName($detail['edge_size_classcategory_id']);
            }

            // 生地色取得
            $productClass = $objProduct->getDetailAndProductsClass($detail['product_class_id']);
            if (!empty($productClass['classcategory_id1'])) {
                $color1 = SC_Helper_DB_Ex::sfGetClassCatName($productClass['classcategory_id1'], true);
                if ($color1 != "") $arrOrderDetail[$key]['classcategory_name1'] .= ":{$color1}";
            }
            if (!empty($productClass['classcategory_id2'])) {
                $color2 = SC_Helper_DB_Ex::sfGetClassCatName($productClass['classcategory_id2'], true);
                if ($color2 != "") $arrOrderDetail[$key]['classcategory_name2'] .= ":{$color2}";
            }

            // フレンジフリルカラー取得
            if ($arrOrderDetail[$key]['frill_id'] > 0) {
                $col = "name";
                $where = "product_id = ?";
                $huriru = $objProduct->getDetail($arrOrderDetail[$key]['frill_id']);
                if ($huriru["name"] != "") $arrOrderDetail[$key]['classcategory_name2'] .= ", " . $huriru["name"];
            }
        }
