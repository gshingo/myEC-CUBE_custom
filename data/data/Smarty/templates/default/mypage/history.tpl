...
<div id="mypagecolumn">
    ...
    <div id="mycontents_area">
        <h3><img src="<!--{$TPL_URLPATH}-->img/mypage/subtitle05.gif" width="515" height="32" alt="購入履歴詳細" /></h3>
        <div class="mycondition_area clearfix">
            <p>
                ...
                <span class="st">お届け時間：&nbsp;</span><!--{$shippingItem.shipping_time|default:'指定なし'|h}--><br />
                <span class="st">お届け日：&nbsp;</span><!--{$shippingItem.shipping_date|default:'指定なし'|h}--><!--{if !$smarty.foreach.shippingItem.last}--><br /><!--{/if}-->
                <!--{/foreach}-->
                <!--{/if}-->
            </p>
            <!--{*
            ...
            *}-->
            ...
            <!--{foreach from=$tpl_arrOrderDetail item=orderDetail}-->
                <tr>
                    ...
                    <td><a<!--{if $orderDetail.enable}--> href="<!--{$smarty.const.P_DETAIL_URLPATH}--><!--{$orderDetail.product_id|u}-->"<!--{/if}-->><!--{$orderDetail.product_name|h}--></a><br /><!--{strip}-->
                        <!--{if $orderDetail.order_made_flg}-->
                            表の色:<!--{$orderDetail.body_color_name|h}--><br />
                            <!--{if $orderDetail.back_color_name != ""}-->裏の色:<!--{$orderDetail.back_color_name|h}--><br /><!--{/if}-->
                            フチの色:<!--{$orderDetail.edge_color_name|h}--><br />
                            フチのサイズ:<!--{$orderDetail.edge_size_name|h}--><br />
                            <!--{if $orderDetail.height_size != ""}-->
                                横：<!--{$orderDetail.width_size|h}-->cm × 縦：<!--{$orderDetail.height_size|h}-->cm
                            <!--{/if}-->
                        <!--{elseif $orderDetail.JBstainplate_flg}-->
                            <!--{if $orderDetail.classcategory_name2 != ""}-->
                                縦：<!--{$orderDetail.classcategory_name2|h}--><br />
                            <!--{/if}-->
                            <!--{if $orderDetail.width_size != ""}-->
                                横：<!--{$orderDetail.width_size}-->cm
                            <!--{/if}-->
                        <!--{else}-->
                            <!--{if $orderDetail.classcategory_name1 != ""}-->
                                <!--{$orderDetail.class_name1|h}-->:<!--{$orderDetail.classcategory_name1|h}-->
                                <!--{if $orderDetail.color1 != ""}-->:<!--{$orderDetail.color1|h}--><!--{/if}-->
                                <br />
                            <!--{/if}-->
                            <!--{if $orderDetail.classcategory_name2 != ""}-->
                                <!--{$orderDetail.class_name2|h}-->:<!--{$orderDetail.classcategory_name2|h}-->
                                <!--{if $orderDetail.color2 != ""}-->:<!--{$orderDetail.color2|h}--><!--{/if}-->
                            <!--{/if}-->
                        <!--{/if}-->
                        <!--{if $orderDetail.frill != ""}--><!--{$orderDetail.frill|h}--><!--{/if}-->
                    <!--{/strip}--></td>
                    <!--{*
                    <td class="alignC">
                    ...
                    *}-->
                    <!--{if  $orderDetail.sell_piece_size != ""}-->
                        <!--{assign var=price value=`$orderDetail.productsClass.price02`}-->
                        <!--{assign var=quantity value=`$orderDetail.sell_piece_size`}-->
                        <!--{assign var=unit value="m"}-->
                    <!--{else}-->
                        <!--{assign var=price value=`$orderDetail.price`}-->
                        <!--{assign var=quantity value=`$orderDetail.quantity`}-->
                        <!--{assign var=unit value=`$orderDetail.unit`}-->
                    <!--{/if}-->
                    <td class="alignR"><!--{$price|sfCalcIncTax|number_format|h}-->円</td>
                    <td class="alignR"><!--{$quantity|h}--><!--{$unit|h}--></td>
            ...
            <tr>
                ...
                <td class="alignR"><span class="price advice"><!--{$tpl_arrOrderData.payment_total|number_format}-->円</span></td>
            </tr>
        </table>
        ...
        <!--{if $smarty.const.USE_POINT !== false}-->
            <table summary="使用ポイント">
                ...
                *}-->
                <tr>
                    <th class="alignC">ご使用ポイント</th>
                    <td class="alignR"><!--{assign var=key value="use_point"}--><!--{$tpl_arrOrderData[$key]|number_format|default:0}--> pt</td>
                </tr>
                <tr>
                    <th class="alignC">今回加算されるポイント</th>
                    <td class="alignR"><!--{$tpl_arrOrderData.add_point|number_format|default:0}--> pt</td>
                </tr>
            </table>
        <!--{/if}-->
        <!-- 使用ポイントここまで -->
        ...
        *}-->
        <div class="btn_area">
            <a href="./<!--{$smarty.const.DIR_INDEX_PATH}-->" onmouseover="chgImg('<!--{$TPL_URLPATH}-->img/common/b_back_on.gif','change');" onmouseout="chgImg('<!--{$TPL_URLPATH}-->img/common/b_back.gif','change');"><img src="<!--{$TPL_URLPATH}-->img/common/b_back.gif" alt="戻る" name="change" id="change" /></a>
        </div>
